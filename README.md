...
[フッター: 気象クレジット / グロウアンドプログレス / v1.6.1]
```
...
| ベント炭素収支（CBI） | 1パネルに炭素収支予測（明日/明後日）と体力指数（過去7日）を統合。**炭素収支とは**（ブログ）リンク・導入文・各項目の **判定ロジック** モーダル |
...
| フッター | 気象データクレジット・グロウアンドプログレスリンク・**v1.6.1** |
...
- セルサイズ・文字サイズを拡大（炭素収支 72×58px、星 14px、CBI 値 12px、体力指数 % 16px）
