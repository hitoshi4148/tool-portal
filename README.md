...
[AI質問箱（入力欄・注意4カ条・右端に Gemma 4 26B）]
...
[フッター: 気象クレジット / グロウアンドプログレス / v1.5.1]
```
...
| AI質問箱 | タイトル下の入力欄から Gemma 4 による芝管理 Q&A（相談室 Worker へ中継）。入力欄下左に注意、右端に **AIモデル: Gemma 4 26B** |
...
| フッター | 気象データクレジット・グロウアンドプログレスリンク・**v1.5.1** |
...
病害リスク予測と同じコンパクトな2列パネルで、ベントグラス向けの炭素収支を表示します。各パネルタイトル横の **判定ロジック** ボタンから、数式と今回の計算値をモーダルで確認できます（×ボタン・背景クリック・Esc で閉じる）。
...
| 芝しごと・AI相談室 | AI | 同一サイト `/aihelpdesk/` へリンク。資料フォルダと音声入力。ポータルと同じ施設 Cookie |
...
- 農薬名・メーカー名 × 病害虫・雑草名（かつ）の検索欄と「検索」ボタン
